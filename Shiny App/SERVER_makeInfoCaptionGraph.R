  ...
  if (input$comparison_mode == "within") {
    ...
    if (input$misspecification == "bothCorrect") {
      ...
      html_text <- paste0("Note: The bars indicate the ", input$performance_graph, " of the ",
                          ifelse(input$datatype == "simulate_diamond1", "ATE", "ATT"), " estimator over 5000 simulation replications. The numbers in brackets to the left of the bars indicate the absolute bias and the standard deviation of the estimator, i.e. [|Bias|, SD]. Red bars represent estimators which use conventional methods to estimate the conditional outcome means (OLS) and the propensity score (Logit). The blue bar represents the OLS estimator regressing the outcome on the treatment indicator and the ten covariates.")
      
    } else if (input$misspecification == "PSCorrect") {
      
      html_text <- paste0("Note: The bars indicate the ", input$performance_graph, " of the ",
                          ifelse(input$datatype == "simulate_diamond1", "ATE", "ATT"), " estimator over 5000 simulation replications. The numbers in brackets to the left of the bars indicate the absolute bias and the standard deviation of the estimator, i.e. [|Bias|, SD]. Green bars represent estimators which use machine learning methods (random forests and elastic net) to estimate the conditional outcome means. Red bars represent estimators which use conventional methods (OLS) to estimate the conditional outcome means. The blue bar represents the simple OLS estimator. The results for random forest (RF) are shown in the left panel, the results for elastic net are shown in the right panel.")
      
    } else if (input$misspecification == "COCorrect") {
      
      html_text <- paste0("Note: The bars indicate the ", input$performance_graph, " of the ",
                          ifelse(input$datatype == "simulate_diamond1", "ATE", "ATT"), " estimator over 5000 simulation replications. The numbers in brackets to the left of the bars indicate the absolute bias and the standard deviation of the estimator, i.e. [|Bias|, SD]. Green bars represent estimators which use machine learning methods (random forests and elastic net) to estimate the propensity score. Red bars represent estimators which use conventional methods (Logit) to estimate the propensity score. The blue bar represents the simple OLS estimator. The results for random forest (RF) are shown in the left panel, the results for elastic net are shown in the right panel.")
      
    } else {
      
      html_text <- paste0("Note: The bars indicate the ", input$performance_graph, " of the ",
                          ifelse(input$datatype == "simulate_diamond1", "ATE", "ATT"), " estimator over 5000 simulation replications. The numbers in brackets to the left of the bars indicate the absolute bias and the standard deviation of the estimator, i.e. [|Bias|, SD]. Green bars represent estimators which use machine learning methods (random forests and elastic net) to estimate the conditional outcome means and the propensity score. Red bars represent estimators which use conventional methods (OLS and Logit) to estimate the conditional outcome means and the propensity score. The blue bar represents the simple OLS estimator. The results for random forest (RF) are shown in the left panel, the results for elastic net are shown in the right panel.")
    ...
    if (input$analysisDim == "linAdd") {
      ...
      text1 <- "Within a subgraph, the x-axis plots three degrees of linearity and additivity. 'L' corresponds to a DGP where both the outcome specification and the treatment specification are linear and additive, i.e. include only the main effects. 'MNL' and 'NL' correspond to DGPs where both the outcome specification and the treatment specification are nonlinear and nonadditive. 'MNL' includes the main effects, one quadratic term and four interaction terms. 'NL' includes the main effects, three quadratic terms, and ten interaction terms."
